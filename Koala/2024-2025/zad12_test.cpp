...
int solve(ofstream& file, vector<pair<int,int>> inOut = {}, map<int, bool> checked = {}, vector<int> arr = {}, int lev = 0){
    if (lev == 9) { // w tym momencie jest permutacja od 1..n
        int i, j;
        int lines_used = 0; // ilość liń które są zajęte

        vector<vector<int>> lines(9, vector<int>(8, 0)); // 9 rzędów po 8 falseów, oznaczających, czy ktoś siedzi na danym miejscu na danej stacji
        ...
        if (lines_used <= 6 && counter[1] == 6 && counter[2] == 5 && counter[3] == 2 && counter[4] == 4 && counter[5] == 3 && counter[6] == 3 && counter[7] == 4 && counter[8] == 2 && counter[9] == 5) {
            for (i = 0; i < 9; ++i) {
                file<< arr[i] << " ";
            }
